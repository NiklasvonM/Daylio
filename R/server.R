server <- shinyServer(function(input, output) {
  ...
  output$correlation <- renderValueBox({
    activity <- cur_activity()
    correlation <- measure_of_dependence(DATA$Mood, DATA[[activity]])
  ...
  output$correlation_matrix <- renderRHandsontable({
    #renderDataTable({
  ...
  output$correlation_matrix_lag <- renderRHandsontable({
    #renderDataTable({
    # tbl <- rhandsontable(MAT_COR)
    # tbl <- hot_cols(tbl, fixedColumnsLeft = 0)
    # tbl <- hot_rows(tbl, fixedRowsTop = 0)
  ...
  output$lookback_pointplot <- renderPlotly({
    ...
    dt[, Count := count_entries_lookback(get(input$activity), n_lookback = input$lookback_pointplot_n - 1)]
    dt <- dt[!is.na(Count)]
    dt[, N := .N, by = c("Count")]
  ...
  output$mood_distribution_by_activity_value <- renderPlotly({
    ...
    dt_fill <- as.data.table(expand.grid(
          Mood = unique(dtPlot$Mood),
          V1 = unique(dtPlot[[activity]])
      ))
    setnames(dt_fill, "V1", activity)
    dtPlot <- merge(dtPlot, dt_fill, by = c("Mood", activity), all = TRUE)
  ...
  output$mood_distribution_wwo_activity <- renderPlotly({
    ...
    if (length(use) > 0) {
      for (activity in use) {
        dt_cur <- dt_cur[get(activity) >= 1]
      }
    }
    if (length(use_not) > 0) {
      for (activity in use_not) {
        dt_cur <- dt_cur[get(activity) <= 0]
      }
    }
    req(nrow(dt_cur) > 0)
    dtPlot <- dt_cur[, .(Dichte = .N / nrow(dt_cur)), by = .(Mood)]
  ...
  output$forcenetwork <- renderVisNetwork({
    ...
    if (file.exists("data/activity_groups.csv")) {
      activity_groups <- fread("activity_groups.csv", encoding = "UTF-8")
      nodes <- merge(nodes, activity_groups, by.x = "label", by.y = "Activity", all.x = TRUE)
  ...
  output$strucutral_break_test <- renderPlotly({
    ...
    dt_cur <- DATA[, c("Day", activity, "Mood"), with = FALSE]
    ...
    for (i in 1:nrow(dtCoeff)) {
      l <- dtCoeff[i]$l
      u <- dtCoeff[i]$u
      dt1 <- dt_cur[l <= Day & Day <= u]
      dt2 <- dt_cur[!dt1, on = names(dt_cur)]
  ...
  output$chord_diagram <- renderChorddiag({
    M <- length(ACTIVITIES)
    dt_cur <- DATA[Mood == input$mood]
    mat <- matrix(0, nrow = (M + 1), ncol = (M + 1))
    rownames(mat) <- colnames(mat) <- c("Mood", ACTIVITIES)
    mat[1, 1] <- nrow(dt_cur) / nrow(DATA)
    for (i in 1:M) {
      mat[i + 1, 1] <- nrow(dt_cur[get(ACTIVITIES[i]) > 0]) / nrow(DATA[get(ACTIVITIES[i]) > 0])
    }
  ...
  output$dependencies <- renderRHandsontable({
    tbl <- rhandsontable(dt_dependencies)
    tbl
  })
  ...
  output$cycles_heatmap <- renderPlotly({
    ...
    min_date <- min(dtCycles$Day)
    offset <- as.integer(min_date) %% cycleLength
    dtCycles[, `day of cycle` := (as.integer(Day) - offset) %% cycleLength]
    dtCycles[, `cycle` := floor(as.integer((Day - min_date)) / cycleLength)]
